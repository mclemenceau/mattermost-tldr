...
from mattermost_tldr.cli import render_post, render_channel_markdown

# A fixed base timestamp: 2026-02-20 10:00:00 UTC
TS_BASE = int(dt.datetime(2026, 2, 20, 10, 0, 0, tzinfo=dt.timezone.utc).timestamp() * 1000)
...
def make_client(usernames=None):
    ...
    client.get_username.side_effect = lambda uid: (usernames or {}).get(uid, uid)
    return client
...
def make_channel(ch_type="O", name="general", display_name="General"):
    return {"id": "chan1", "name": name, "display_name": display_name, "type": ch_type}
...
class TestRenderPost:
    ...
    def test_time_in_brackets(self):
        ...
        ts = int(dt.datetime(2026, 2, 20, 14, 35, 0, tzinfo=dt.timezone.utc).timestamp() * 1000)
...
class TestRenderChannelMarkdown:
    def test_no_posts_shows_placeholder(self):
        client = make_client()
        result = render_channel_markdown(make_channel(), [], client, DATE_BASE, DATE_BASE)
        assert "*No messages in this period.*" in result

    def test_channel_header_type(self):
        client = make_client()
        result = render_channel_markdown(make_channel(ch_type="O"), [], client, DATE_BASE, DATE_BASE)
    ...
    def test_dm_header_type(self):
        ...
        result = render_channel_markdown(ch, [], client, DATE_BASE, DATE_BASE, display_name="DM with alice")
    ...
    def test_period_in_header(self):
        ...
        result = render_channel_markdown(
            make_channel(), [], client,
            dt.date(2026, 2, 1), dt.date(2026, 2, 20),
        )
    ...
    def test_display_name_override(self):
        ...
        result = render_channel_markdown(ch, [], client, DATE_BASE, DATE_BASE, display_name="Override")
    ...
    def test_post_rendered(self):
        ...
        result = render_channel_markdown(make_channel(), [post], client, DATE_BASE, DATE_BASE)
    ...
    def test_thread_reply_nested_after_root(self):
        ...
        reply = make_post("reply1", "u2", "Reply message", create_at=TS_BASE + 60_000, root_id="root1")
        result = render_channel_markdown(make_channel(), [root, reply], client, DATE_BASE, DATE_BASE)
    ...
    def test_unrelated_posts_not_nested(self):
        ...
        result = render_channel_markdown(make_channel(), [p1, p2], client, DATE_BASE, DATE_BASE)
        # Neither post should have the reply indent
        lines = result.splitlines()
        alice_line = next(l for l in lines if "First post" in l)
        bob_line = next(l for l in lines if "Second post" in l)
        assert not alice_line.startswith("  ↳")
        assert not bob_line.startswith("  ↳")

    def test_posts_grouped_by_day(self):
        client = make_client({"u1": "alice"})
        ts_thu = int(dt.datetime(2026, 2, 19, 10, 0, 0, tzinfo=dt.timezone.utc).timestamp() * 1000)
        ts_fri = int(dt.datetime(2026, 2, 20, 10, 0, 0, tzinfo=dt.timezone.utc).timestamp() * 1000)
        p1 = make_post("p1", "u1", "Thursday message", create_at=ts_thu)
        p2 = make_post("p2", "u1", "Friday message", create_at=ts_fri)
        result = render_channel_markdown(
            make_channel(), [p1, p2], client,
    ...
    def test_empty_post_message_excluded(self):
        ...
        result = render_channel_markdown(make_channel(), [empty_post], client, DATE_BASE, DATE_BASE)
